...
@implementation PlayerAudioBuffer
...
{
    NSLog(@"Pushing audio sample to buffer");
    int pushedBytes = 0;
        ...
        if (_dataByteCapacity > _dataByteSize) {
            ...
            while (toPush > 0) {
                ...
                if (_dataByteCapacity <= _dataByteSize) {
                    ...
                    NSLog(@"Waiting to push audio samples");
                    [_condition wait];
                }
            ...
            NSLog(@"Finish pushing audio sample");
            break;
        }else if(!blocked){
            NSLog(@"No sample pushed");
            pushedBytes = 0;
            break;
        }else{
            NSLog(@"Waiting to push audio samples");
            ...
            *outPts = _currentPts;
            while (toRead > 0) {
                ...
                if (_dataByteSize <= 0) {
                    ...
                    NSLog(@"Waiting to pop more audio samples");
                    [_condition wait];
                }
            ...
            NSLog(@"Popped audio sample");
            break;
        }else if (!blocked){
            NSLog(@"NO samples popped");
            readBytes = 0;
            break;
        }else{
            NSLog(@"Waiting to pop audio samples");
            [_condition wait];
        }
    }
